...
const whitelistStore = new Map<number, UserWhitelist>();
...
function normalizeIp(ip: string): string {
  ...
  const hasSingleColon = withoutBracket.indexOf(':') === withoutBracket.lastIndexOf(':');
  const maybeIpv4WithPort = hasSingleColon ? withoutBracket.replace(/:\d+$/, '') : withoutBracket;
...
export function getWhitelist(userId: number): UserWhitelist {
  return whitelistStore.get(userId) ?? {};
}

export function setWhitelist(userId: number, data: UserWhitelist): UserWhitelist {
...
export function isRequestIpAllowed(userId: number, requestIp: string | null): boolean {
...
export function validateWhitelistInput(ipv4?: string, ipv6?: string): string | null {
